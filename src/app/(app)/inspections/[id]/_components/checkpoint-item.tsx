...
import type { Checkpoint, ReportCheckpoint } from '@/lib/types';
...
interface CheckpointItemProps {
  checkpoint: Checkpoint & ReportCheckpoint & { id: string };
  form: UseFormReturn<any>;
  index: number;
}

const statusIcons = {
  'Conform': <CheckCircle className="text-green-500" />,
  'Non-Conform': <XCircle className="text-destructive" />,
}

// Create a new component to isolate the useWatch hook
function CheckpointStatusIcon({ control, index }: { control: any; index: number }) {
  const status = useWatch({
    control,
    name: `checkpoints.${index}.status`,
  });

  return <div className='w-5'>{statusIcons[status as keyof typeof statusIcons]}</div>;
}


export function CheckpointItem({ checkpoint, form, index }: CheckpointItemProps) {
  ...
  return (
    <AccordionItem value={`chk-item-${index}`} className="border bg-card rounded-lg mb-4 shadow-sm print-new-page">
      <AccordionTrigger className="hover:bg-muted/50 px-4 rounded-t-lg data-[state=closed]:rounded-b-lg data-[state=open]:border-b">
        <div className="flex items-center gap-4">
          <CheckpointStatusIcon control={form.control} index={index} />
          <span className="font-semibold text-base">{checkpoint.title}</span>
