...
import { PageHeader } from '@/components/page-header';
import { checkpoints as allCheckpoints, clients, samplePurposes } from '@/lib/data';
import { products } from '@/lib/products';
import type { Inspection } from '@/lib/types';
  ...
  function onSubmit(values: NewInspectionFormValues) {
    ...
    const relevantCheckpoints = allCheckpoints.filter((checkpoint) => {
      const clientMatch =
        ...
        checkpoint.clients.includes(values.client);

      const categoryMatch =
        !checkpoint.productCategories?.length ||
        checkpoint.productCategories.includes(product.category);

      const purposeMatch =
        !checkpoint.samplePurposes?.length ||
        checkpoint.samplePurposes.includes(values.samplePurpose);
